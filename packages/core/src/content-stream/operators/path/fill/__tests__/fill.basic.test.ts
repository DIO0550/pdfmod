...
test("pendingClip が some のとき `f` は消費して none に戻す", () => {
  const ctx = buildContextWithGraphicsState(
    stateWith(some(ClippingRule.nonzero()), [PathSegment.moveTo(0, 0)]),
  );
...
test("pendingClip が none のとき `f` は従来どおり path だけをリセットする", () => {
  const ctx = buildContextWithGraphicsState(
...
test("空 path でも pendingClip が some なら `f` は消費する", () => {
  const ctx = buildContextWithGraphicsState(
...
test("空 path かつ pendingClip が none なら `f` は完全な no-op になる", () => {
