...
test("ページに /MediaBox が直接あれば継承値を無視する", () => {
...
test("ページに /MediaBox がなく継承に存在すれば継承値を使う", () => {
...
test("ページも継承も /MediaBox 未定義なら MEDIABOX_NOT_FOUND を返す", () => {
  const pageDict = makePageDict({});
...
test("/Rotate が 45 の場合 90 に丸められ INVALID_ROTATE 警告が出る", () => {
...
test("/Rotate が 450 の場合 90 に射影され警告なし", () => {
  const outcome = unwrapOk(
...
test("/Rotate が 90 の場合 警告なしで 90 を採用する", () => {
  const outcome = unwrapOk(
...
test("/Rotate が -90 の場合 270 に正規化され警告なし", () => {
  const outcome = unwrapOk(
    runRotate({ type: "integer", value: -90 }, {}, { rotate: -90 }),
  );
...
test("/Rotate が -450 の場合 270 に正規化され警告なし", () => {
  const outcome = unwrapOk(
    runRotate({ type: "integer", value: -450 }, {}, { rotate: -450 }),
  );
  expect(outcome.page.rotate).toBe(270);
...
test("/Rotate キーが存在しない場合は警告なしで 0", () => {
...
test("/Rotate が -45 なら INVALID_ROTATE + 0", () => {
  const outcome = unwrapOk(
    runRotate({ type: "integer", value: -45 }, {}, { rotate: -45 }),
  );
  expect(outcome.page.rotate).toBe(0);
...
test("ページ /Rotate キー不在、継承 /Rotate=90 なら警告なしで 90", () => {
  const pageDict = makePageDict({ mediaBox: [0, 0, 10, 10] });
  const outcome = unwrapOk(
    InheritanceResolver.resolve(
      pageDict,
