...
import { PdfHeader } from "../pdf-header/index";
...
export class PdfDocument {
  ...
  static async load(
    ...
    const headerResult = PdfHeader.parse(data);
    if (!headerResult.ok) {
      return headerResult;
    }
    ...
    const pdfData =
      (header.offset as number) > 0
        ? data.subarray(header.offset as number)
        : data;
    ...
    const xrefResolution = await resolveXRefStructure(pdfData, emitWarnings);
