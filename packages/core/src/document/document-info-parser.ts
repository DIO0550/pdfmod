...
/**
 * 日時フィールド共通のリーダ。値の型チェック → 文字列復号 → {@link parsePdfDate} を束ねる。
 *
 ...
 * - 辞書あり → 9 フィールドを抽出して返す
 *
 * `Result.err` を返すのは現状なし（resolver は契約上 Promise を reject せず err を Ok 化して戻す）。
 */
export const DocumentInfoParser = {
   ...
   * @returns 抽出結果と警告を含む `Ok`。現状 `Err` 経路は使用していない
   */
  async parse(
