...
test("integer 単体では integer を返す", () => {
  const result = DirectObject.parse(btOf("5 (str)"), 0, 0);
  assert(result.ok);
  expect(result.value).toEqual({ type: "integer", value: 5 });
});

test("N G R パターンで indirect-ref を返す", () => {
  const result = DirectObject.parse(btOf("5 0 R"), 0, 0);
  assert(result.ok);
