...
test("LF 改行後のストリームデータが正しく切り出される", () => {
  const data = enc("\nhello\nendstream");
  const result = StreamObject.extract(data, ByteOffset.of(0), 0, emptyDict, 5);
  assert(result.ok);
...
test("length が負でエラー", () => {
  const data = enc("\nhello\nendstream");
  const result = StreamObject.extract(data, ByteOffset.of(0), 0, emptyDict, -1);
  assert(!result.ok);
  expect(result.error.code).toBe("OBJECT_PARSE_STREAM_LENGTH");
});
...
test("データ範囲超過でエラー", () => {
  const data = enc("\nhello\nendstream");
  const result = StreamObject.extract(
    data,
    ByteOffset.of(0),
    0,
    emptyDict,
