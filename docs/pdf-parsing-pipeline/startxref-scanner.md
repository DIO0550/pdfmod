...
1. **末尾領域の算出**: `tailStart = Math.max(0, data.length - 1024)` で末尾1024バイトの開始位置を決定
2. **%%EOF 逆方向検索**: 末尾からバイト列 `[0x25, 0x25, 0x45, 0x4F, 0x46]` を逆方向に検索。最後の %%EOF を自動的に検出
3. **startxref 逆方向検索**: %%EOF の位置から上方向にバイト列 `"startxref"` を検索。トークン境界チェック（前にホワイトスペースまたはデータ先頭）と後続の数字列の存在を検証
4. **コメントスキップ**: PDF仕様ではコメント（`%` から行末まで）はホワイトスペースと同等に扱われるため（ISO 32000 7.2.4）、`startxref` と数値の間にコメントがある場合も正しく処理
5. **オフセット値パース**: `startxref` キーワード末尾からホワイトスペース/コメントをスキップし、連続するASCII数字を収集して整数に変換
