...
> **注（コード断片の位置づけ）**: 01〜09章に含まれる Rust 風のコード断片・擬似コードは、ISO 32000 の概念を説明するための**非規範の例示**であり、実装言語・API・型を規定するものではない。規範となる機能仕様は `docs/pdf-parsing-pipeline/` に言語中立の表・散文で定義する。
