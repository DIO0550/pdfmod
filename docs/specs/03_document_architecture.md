...
ISO 32000-1 §7.3.10 により、継承属性の値も配列の要素も間接参照でありうる。
以下はいずれも適法であり、解決してから読み取る必要がある。

| 記述例 | 解決対象 |
|:-------|:---------|
| `/MediaBox 10 0 R` | 値自体 |
| `/MediaBox [0 0 10 0 R 792]` | 配列要素 |
| `/Kids 20 0 R` | 値自体（要素は子への参照なので解決しない） |

解決は 1 段だけ行い、多段参照・循環参照の扱いはオブジェクト解決層に委ねる。

局所値が読み取れなかった場合（参照を辿れない・型が不正）は、その属性を
**未設定として扱い祖先の継承値へフォールバックする**。局所値が無効であることは
警告として報告し、致命的エラーにはしない。`/MediaBox` については祖先にも
有効な値が無い場合にのみ `MEDIABOX_NOT_FOUND` となる。
